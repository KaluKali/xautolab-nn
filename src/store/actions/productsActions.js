...
export const getProductsNext = (pointer, lastDoc, perPage) => dispatch => {
  dispatch(getProductsStarted());
  if (lastDoc.length) {
    pointer.startAfter(lastDoc[lastDoc.length - 1]).limit(perPage)
      .get()
      .then(docs => {
        const products = docs.docs.map((doc) => ({ _id: doc.id, ...doc.data() }));
...
export const getProductsPrev = (pointer, lastDoc, perPage) => dispatch => {
  dispatch(getProductsStarted());
  if (lastDoc) {
